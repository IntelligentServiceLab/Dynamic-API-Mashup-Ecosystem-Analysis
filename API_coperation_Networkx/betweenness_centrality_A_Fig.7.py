...
if __name__ == "__main__":
    ...
    list_network = [network_05_to_08, network_05_to_11, network_05_to_14, network_05_to_17, network_05_to_19, network_05_to_21]
    ...
    for i in range(1, 7):
        ...
        if i >= 1 and i <= 3:
            if i == 1:
                ...
                ax.text(140, 0.135, anotation[i - 1], fontdict=font2)
            if i == 2:
                ax.text(270, 0.15, anotation[i - 1], fontdict=font2)
            if i == 3:
                ax.text(300, 0.13, anotation[i - 1], fontdict=font2)
        if i >= 4 and i <= 6:
            if i == 4:
                ax.set_ylabel("betweenness centrality", font1)
                ax.text(300, 0.125, anotation[i - 1], fontdict=font2)
            if i == 5:
                ax.text(300, 0.125, anotation[i - 1], fontdict=font2)
            if i == 6:
                ax.text(300, 0.124, anotation[i - 1], fontdict=font2)
    ...
    # plt.savefig('E:/IntelligentServiceLab/动态分析论文/picture/3.2 API_coperation/betweenness_centrality.png', format='png', dpi=600)
    # plt.savefig('E:/IntelligentServiceLab/动态分析论文/picture/3.2 API_coperation/betweenness_centrality.tif', format='tif', dpi=600)
    plt.savefig('E:/IntelligentServiceLab/动态分析论文/picture/3.2 API_coperation/betweenness_centrality.svg', format='svg', bbox_inches='tight', dpi=600)
    plt.show()