...
def draw_degree_distribution():
    ...
    list_network = [category_05_to_08, category_05_to_11, category_05_to_14, category_05_to_17, category_05_to_19,
                    category_05_to_21]
    ...
    # 得出每个节点的度将其
    for i in range(1, 7):
        d = dict(nx.degree(list_network[i - 1]))
        x = list(range(max(d.values()) + 1))
        y = [j for j in nx.degree_histogram(list_network[i - 1])]
        ...
        if i >= 1 and i <= 3:
            ax.set_ylim(0, 2)
            ax.text(0.3, 1, formula, fontdict=font3)
            ax.text(2.5, 1.5, anotation[i - 1], fontdict=font2)
            if i == 1:
                ax.set_ylabel("lgN", font1)
        if i >= 4 and i <= 6:
            ax.set_ylim(0, 2)
            ax.text(0.3, 1, formula, fontdict=font3)
            ax.text(2.5, 1.5, anotation[i - 1], fontdict=font2)
            if i == 4:
                ax.set_ylabel("lgN", font1)
            ax.set_xlabel("lgK", font1)

        # 设置刻度线
        ax.tick_params(which='both', direction='in', width=3, colors='black', labelsize=11)
        ...
        ax.plot(x, ynew, 'r')
    # plt.savefig('E:/IntelligentServiceLab/动态分析论文/picture/3.3 category_coperation/degree_distribution_category.tif', format='tif', dpi=600)
    # plt.savefig('E:/IntelligentServiceLab/动态分析论文/picture/3.3 category_coperation/degree_distribution_category.png', format='png', dpi=600)
    plt.savefig('E:/IntelligentServiceLab/动态分析论文/picture/3.3 category_coperation/degree_distribution_category.svg', format='svg', bbox_inches='tight', dpi=600)
    plt.show()
...
if __name__ == "__main__":
    ...
    # anotation = ['(a)', '(b)', '(c)', '(d)', '(e)', '(f)']
    #
    # list_network = [category_05_to_11, category_05_to_13, category_05_to_15, category_05_to_17, category_05_to_19, category_05_to_21]
    #
    # # 进行度的分析
    # # 得出每个节点的度将其
    # for i in range(1, 7):
    ...
    #         ax.text(2.5, 2.5, anotation[i - 1], fontdict=font2)
    #         if i == 1:
    #             ax.set_ylabel("lgN", font1)
    #     if i >= 4 and i <= 6:
    #         ax.set_ylim(0, 3)
    ...
    #
    #     # 设置刻度线
    #     ax.tick_params(which='both', direction='in', width=1, colors='gray')
    #
    #     # 设置坐标轴轴线的颜色
